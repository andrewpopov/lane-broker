...
import { evaluateNewAdmission, recordAdmission, logAdmissionDecision } from './admission.js';
...
/** Record that a ticket other than the literal FIFO head was just allowed to
 *  start ahead of it (a "skip event" — see conflictSkipLimit in tryStart).
 *  The count is keyed on the head's own id and resets automatically the
 *  next time a different ticket occupies the head slot (readSkipState
 *  above returns 0 for a headId mismatch), so no explicit reset is needed
 *  once the head itself finally starts or is cancelled. Best-effort: a
 *  failed write here should never block scheduling, only cost an
 *  undercounted skip (worst case, one extra skip beyond the configured
 *  limit) — the same tolerance as recordAdmission in admission.js. */
...
export async function tryStart(root, ticket, globalCfg, loadSampler, cpuSampler) {
  return withLock(root, () => {
    ...
    if (position === -1) {
      return { started: false, reason: 'not-head', position: null, queueLength: queue.length };
    }
    ...
    // effects. Only the selected candidate pays for a real evaluation.
    ...
    if (!headTicket) {
      ...
      return { started: false, reason: 'not-head', position: position + 1, queueLength: queue.length };
    }
    ...
    const cpuDecision = evaluateNewAdmission(root, globalCfg, ticket, held, cpuSampler);
    const logBase = { candidateId: ticket.id, mode: globalCfg.schedulerMode, ...cpuDecision };

    if (gate.closed) {
      logAdmissionDecision(root, { ...logBase, currentDecision: 'deny', currentReason: 'load-gate-closed' });
    ...
    if (runningWeight + ticket.weight > globalCfg.capacity) {
      logAdmissionDecision(root, { ...logBase, currentDecision: 'deny', currentReason: 'capacity' });
    ...
    if (globalCfg.schedulerMode === 'active' && !cpuDecision.admit) {
      logAdmissionDecision(root, { ...logBase, currentDecision: 'start', currentReason: 'ok' });
      return { started: false, reason: 'cpu-admission', cpuReason: cpuDecision.reason, projectedBusy: cpuDecision.projectedBusy, budget: cpuDecision.budget };
    }
    logAdmissionDecision(root, { ...logBase, currentDecision: 'start', currentReason: 'ok' });
    if (ticket.id !== headTicket.id && headConflicted) {
      // This ticket is genuinely skipping ahead of the still-blocked head —
      // count it toward conflictSkipLimit above.
      recordSkip(root, headTicket.id);
    }
