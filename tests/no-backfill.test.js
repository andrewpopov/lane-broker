...
test("strict FIFO: a heavy ticket at the head blocks a lighter, non-conflicting ticket behind it (no backfill)", async () => {
  ...
  writeRepoConfig(repoDir, {
    version: 1,
    lanes: { heavy: { weight: 2 }, light: { weight: 1 } },
  });
  ...
  await waitFor(
    () => {
      try {
        return (
          fs.readdirSync(paths(state).leases).filter((n) => n.endsWith(".json"))
            .length > 0
        );
      } catch {
        return false;
    ...
    // 60s, not 30s: this waits for a real detached supervisor to spawn AND be admitted. Under a
    // full-suite run on a loaded machine that regularly exceeds 30s, and it fails identically on
    // unmodified main, so it is the wait that is too short rather than anything under test. Same
    // budget as admission-integration.test.js's equivalent wait.
    { timeoutMs: 60000 },
  );
  ...
  const light = laneSpawn(
    [
      ...
      "sh",
      "-c",
      `echo light >> "${startFile}"; sleep 0.1`,
    ],
    { env, cwd: repoDir },
  );
  ...
  await sleep(400);
  const linesWhileHeavyRuns = fs.existsSync(startFile)
    ? fs.readFileSync(startFile, "utf8").trim().split("\n")
    : [];
  assert.deepEqual(
    linesWhileHeavyRuns,
    ["heavy"],
    "the light ticket must not backfill ahead of the heavy head",
  );
  ...
  assert.deepEqual(
    finalLines,
    ["heavy", "light"],
    "the light ticket must start only after the heavy one releases",
  );
});
